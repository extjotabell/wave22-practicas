...
public class Main {
    public static void main(String[] args) {
        ...
        cochesFiltrados.stream().forEach(System.out::println);

        System.out.println("\n----------------------Vehiculo >= 1000-----------------------------");
        List<Vehiculo> cochesFiltrados2 = nuevoGarage.getVehiculos().stream()
                .filter((x) -> x.getCosto() >= 1000).collect(Collectors.toList());

        cochesFiltrados2.stream().forEach(System.out::println);


        System.out.println("\n----------------------Promedio-----------------------------");

        OptionalDouble promedio = nuevoGarage.getVehiculos().stream()
