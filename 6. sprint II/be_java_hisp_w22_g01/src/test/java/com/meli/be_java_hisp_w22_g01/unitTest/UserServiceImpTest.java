...
class UserServiceImpTest {
    ...
    void t0002Ok(){
        ...
        MessageDTO messageExpected = new MessageDTO("El usuario userId: " + paramUserId + " ha dejado de seguir a userId: " + paramUserIdToUnfollow);

        when(userRepository.getAll()).thenReturn(UtilTestGenerator.get3UserWithFollowers());

        // ACT
        MessageDTO messageUnfollow = userService.unfollow(paramUserId, paramUserIdToUnfollow);

        // ASSERT
        Assertions.assertEquals(messageExpected, messageUnfollow);
