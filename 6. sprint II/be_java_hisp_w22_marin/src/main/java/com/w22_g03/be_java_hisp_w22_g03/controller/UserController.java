...
public class UserController {
    ...
    public ResponseEntity<ResponseDTO> followASeller(@PathVariable @NotNull(message = "El  id no puede estar vacío.") @Min(value = 1, message = "El id debe ser mayor a cero") Integer userId,
    ...
    public ResponseEntity<NumberOfFollowersDTO> amountOfFollowersForSeller(@PathVariable @NotNull(message = "El  id no puede estar vacío.") @Min(value = 1, message = "El id debe ser mayor a cero") Integer userId){
        return new ResponseEntity<>(userService.getNumberOfFollowers(userId), HttpStatus.OK);
    }

    //US 0003
    @GetMapping("/{userId}/followers/list")
    public ResponseEntity<FollowerDTO> getFollowers(@PathVariable @NotNull(message = "El  id no puede estar vacío.") @Min(value = 1, message = "El id debe ser mayor a cero") Integer userId,
    ...
    public ResponseEntity<FollowedDTO> getFollowed(@PathVariable @NotNull(message = "El  id no puede estar vacío.") @Min(value = 1, message = "El id debe ser mayor a cero") Integer userId,
                                                   @RequestParam(required = false) String order){
        return new ResponseEntity<>(userService.getFollowed(userId, order),HttpStatus.OK);}

    //US 0007
    @PostMapping("/{userId}/unfollow/{userIdToUnfollow}")
    public ResponseEntity<ResponseDTO> unfollowASeller(@PathVariable @NotNull(message = "El  id no puede estar vacío.") @Min(value = 1, message = "El id debe ser mayor a cero") Integer userId,
