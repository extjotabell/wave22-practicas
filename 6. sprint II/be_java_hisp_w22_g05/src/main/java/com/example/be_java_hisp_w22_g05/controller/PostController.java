...
public class PostController {
    ...
    @PostMapping("/post")
    public ResponseEntity<Void> newPost(@RequestBody @Valid PostDto postDto){
        postService.saveNewPost(postDto);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @GetMapping("/followed/{userId}/list")
    public ResponseEntity<List<PostDto>> getListPostsFromSellersFollowed(@PathVariable @Min(value = 1, message = "El userId debe ser mayor que 0") int userId, @RequestParam(required = false) String order){
