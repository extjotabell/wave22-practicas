...
public class PostService implements IPostService {
    ...
    @Override
    public void saveNewPost(PostDto postDto, boolean isPromo) {
        ...
        //Si no es promoción, el hasPromo y el descuento quedan en null
        Post newPost = new Post(postDto.getId(), postDate, postDto.getCategory(), user, product, postDto.getPrice(), isPromo? postDto.getHasPromo() : null, isPromo? postDto.getDiscount() : null);
    ...
    public void saveNewPromoPost(PostDto postDto) {
        //Verifica que estén completos y correctos hasPromo y discount
        boolean validPost = validatePost(postDto);

        if(validPost){
            saveNewPost(postDto, true);
        }else {
            throw new BadRequestException("Descuento incorrecto");
        }
    }
    ...
    @Override
    public PromoPostDto getPromoPostsCountFromSeller(int id) {
        List<Post> postList = postRepository.findPromoPostsBySeller(id);

        if(postList.isEmpty()) throw new NotFoundException("No existen publicaciones en promoción para el id "+id);

        return new PromoPostDto(id, postList.get(0).getUser().getName(), postList.size());
    }

    @Override
    public PromoPostListDto getPromoPostsFromSeller(int id) {
        List<Post> postList = postRepository.findPromoPostsBySeller(id);

        if(postList.isEmpty()) throw new NotFoundException("El vendedor no tiene productos en promoción");

        //Mapeo la lista de posts
        List<PostDto> postDtoList = postList.stream().map(postMapper::toPostDtoPromo).toList();

        //Obtengo el username de la 1ra lista
        String username = postList.get(0).getUser().getName();

        return new PromoPostListDto(id, username, postDtoList);
    }
    ...
    private boolean validatePost(PostDto postDto) {
        if(!postDto.getHasPromo() || postDto.getDiscount() == null || postDto.getDiscount() <= 0){
            return false;
        }
        return true;
    }
