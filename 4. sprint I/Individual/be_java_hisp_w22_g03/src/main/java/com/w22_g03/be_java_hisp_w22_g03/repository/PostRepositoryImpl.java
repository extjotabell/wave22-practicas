...
public class PostRepositoryImpl implements PostRepository {
    ...
    @Override
    public int countPostsWithDiscounts(long id) {
       User myUser = this.userRepository.findById(id);
       return (int) myUser.getPosts().stream().filter(Post::isHasPromo).count();
    }
    ...
    @Override
    public List<Post> getPostsWithPromos(long id) {
        return this.userRepository.findById(id).getPosts().stream().filter(Post::isHasPromo).collect(Collectors.toList());
    }
}
