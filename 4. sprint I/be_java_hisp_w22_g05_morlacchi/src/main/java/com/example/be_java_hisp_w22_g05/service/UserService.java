...
public class UserService implements IUserService {
    ...
    @Override
    public List<UserNumberFollowersDto> mostPopular() {
        List<User> sellers = userRepository.findAllUsers();
        sellers.sort((u1,u2) -> u2.getFollower().size() - u1.getFollower().size());
        List<UserNumberFollowersDto>  sellersOrderByFollowers= sellers.stream().map(s-> new UserNumberFollowersDto(s.getId(),s.getName(),s.getFollower().size())).toList();
        return sellersOrderByFollowers;
    }
