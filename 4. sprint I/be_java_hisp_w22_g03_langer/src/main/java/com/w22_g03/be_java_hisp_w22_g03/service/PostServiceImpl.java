...
public class PostServiceImpl implements PostService {
    ...
    @Override
    public PostDTO addPromoPost(PostDTO postDTO) {
        if (!postDTO.isHasPromo()){
            throw new BadRequestException("Promo post isn't promo");
        }
        if (postDTO.getDiscount() == 0.0){
            throw new BadRequestException("Promo post doesn't have a discount");
        }
        return addPost(postDTO);
    }

    @Override
    public NumberOfPromosDTO getPromoCount(int userId) {
        User user = userService.findById(userId);
        return new NumberOfPromosDTO(userId, user.getUsername(), postRepository.countPromoPostsFromVendor(user));
    }

    List<PostDTO> filterPostDTOSince(List<PostDTO> listOfPostDTO, String since){
        if (since == null){
        ...
        for (PostDTO p : listOfPostDTO ){
            if (p.getDate().isAfter(sinceDate)) {
                filteredList.add(p);
            }
        }
        return filteredList;
    ...
    List<PostDTO> filterPostDTOUpTo(List<PostDTO> listOfPostDTO, String upTo){
        if (upTo == null){
            return listOfPostDTO;
        }
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        List<PostDTO> filteredList = new ArrayList<>();
        LocalDate upToDate = LocalDate.parse(upTo, formatter);
        for (PostDTO p : listOfPostDTO ){
            if (p.getDate().isBefore(upToDate)) {
                filteredList.add(p);
            }
        }
        return filteredList;
    ...
    @Override
    public PostsBySellerDTO getPromoList(int userId, String since, String upTo) {
        List<PostDTO> unFilteredList = getFullPromoList(userId).getPosts();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        if (since != null && upTo != null && LocalDate.parse(since, formatter).isAfter(LocalDate.parse(upTo, formatter))){
            throw new BadRequestException("Since and UpTo times are incompatible");
        }

        return new PostsBySellerDTO(userId, filterPostDTOUpTo(filterPostDTOSince(unFilteredList, since), upTo));
    }

    private PostsBySellerDTO getFullPromoList(int userId){
        User user = userService.findById(userId);
        List<PostDTO> postDTOList = new ArrayList<>();
        for (Post post : postRepository.getAllPromoPostsFromVendor(user)){
            postDTOList.add(mapPostToPostDto(post));
        }
        return new PostsBySellerDTO(userId, postDTOList);
    }
