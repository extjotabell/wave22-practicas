...
public class PostController {
    ...
    public ResponseEntity<?> newPromoPost(@RequestBody PostDto postDto){
        postService.saveNewPost(postDto, true);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<?> getAmountOfPromosByUser(@RequestParam int userId){
        return new ResponseEntity<>(postService.getAmountOfPromos(userId), HttpStatus.OK);
    }
}
